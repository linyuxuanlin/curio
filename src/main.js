...
function createCard(c, index) {
 ...
 const flip = el('button','flip-button','翻到背面看正文与来源 ↗'); flip.type='button'; flip.addEventListener('click',()=>card.classList.add('is-flipped')); frontBody.append(flip);
 ...
 const flipBack = el('button','flip-button flip-back','翻回正面 ↩'); flipBack.type='button'; flipBack.addEventListener('click',()=>card.classList.remove('is-flipped')); back.append(flipBack);
...
function bindDrag(card) {
 ...
 card.addEventListener('pointermove',e=> { if(!drag||!e.isPrimary)return; let dx=e.clientX-drag.x,dy=e.clientY-drag.y; if(!drag.locked){const ax=Math.abs(dx),ay=Math.abs(dy);if(Math.max(ax,ay)<8)return;if(ay>ax*1.15){drag=null;return;}if(ax>8){drag.locked=true;card.setPointerCapture(e.pointerId);card.classList.add('dragging');}} if(!drag.locked)return; e.preventDefault(); drag.v=(e.clientX-drag.lastX)/Math.max(1,e.timeStamp-drag.lastT); drag.lastX=e.clientX;drag.lastT=e.timeStamp;drag.dx=dx;drag.dy=dy*.28;card.style.transform=`translate(${dx}px,${drag.dy}px) rotate(${dx*.055}deg)`; },{passive:false});
