...
function bindDrag(card) {
 ...
 card.addEventListener('pointermove',e=> { if(!drag||!e.isPrimary)return; const dx=e.clientX-drag.x,dy=e.clientY-drag.y; if(!drag.locked){if(Math.max(Math.abs(dx),Math.abs(dy))<8)return;drag.locked=true;card.setPointerCapture(e.pointerId);card.classList.add('dragging');} e.preventDefault(); const dt=Math.max(1,e.timeStamp-drag.lastT);drag.v=Math.hypot(e.clientX-drag.lastX,e.clientY-drag.lastY)/dt;drag.lastX=e.clientX;drag.lastY=e.clientY;drag.lastT=e.timeStamp;drag.dx=dx;drag.dy=dy;drag.rotation=dx*.045-dy*.02;card.style.transform=`translate(${dx}px,${dy}px) rotate(${drag.rotation}deg)`; },{passive:false});
 const end=(e,cancel=false)=> { if(!drag)return; const d=drag;drag=null;card.classList.remove('dragging');if(card.hasPointerCapture(e.pointerId))card.releasePointerCapture(e.pointerId);if(!cancel&&d.locked&&shouldDismiss(Math.hypot(d.dx,d.dy),e.timeStamp-d.lastT<100?d.v:0,card.offsetWidth))dismiss(1,d.dx,d.dy,d.rotation);else{card.style.transition=reduced?'none':'transform 460ms cubic-bezier(.18,1.5,.35,1)';card.style.transform='';setTimeout(()=>card.style.transition='',470);} };
