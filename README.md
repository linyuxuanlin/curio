...
- 初次打开：最新入库的未读卡片优先。
- 卡片采用竖向标准信用卡比例（宽高约 0.63:1）；正面看图和简介，点击整张卡片以 3D 方式翻到背面分页查看完整正文、来源与图片署名；聚焦卡片后也可按 Enter / 空格翻面。
...
- 在“关于 Curio”中可主动开启或关闭桌面更新通知；每 5 分钟检查新内容，后台打开的页面也会检查，同一批更新不重复通知。关闭页面后不会定时推送，浏览器休眠时可能延迟。
...
卡片背面不重复正面标题和简介；正文最多两段、每段80字、合计140字，构建时强制校验。手机短屏仍可分页，来源标题与链接作为一组展示。
