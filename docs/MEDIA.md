...
构建时 `scripts/prepare-images.mjs` 将原图等比例缩小为 480 和 960 像素以内的 WebP，保留完整画面，不修改事实内容。`public/media/optimized/` 按源 URL（本地图片按文件字节）与处理版本生成哈希文件名；已提交的图片可在没有外网的情况下复用。生成 feed 的 src/srcset 只指向本站图片，署名、出处与许可保留原值。新内容仍可提供原始 HTTPS 图片地址，构建器负责下载与转换；无法取得有效图片时构建失败，保持上次正常部署，不能发布空图。后续构建也会尝试复用上一生产部署的同名优化文件。

Wikimedia 图片直接获取标准缩略图，失败时尝试原图；MDPI 图使用其图片 CDN。开发时如需代理，可设置 HTTPS_PROXY 为 HTTP 代理地址。切勿把临时签名链接写入内容卡片。
