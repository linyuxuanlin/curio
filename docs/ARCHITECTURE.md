...
卡片采用竖版标准信用卡比例（宽高约 0.63:1），正面固定展示图片、标题和简介；点击整张卡片通过 3D 翻面查看正文、来源与图片署名。页面和卡片都不提供滚动区域，避免移动端手势与页面滚动冲突。仅成功划走或点击下一张才记已读；浏览到、翻面或拖动回弹不会记已读。撤回恢复此前已读/最近阅读状态。多标签页同步读取 localStorage 的变化。读完最后一张未读卡片时触发一次烟花动画。
