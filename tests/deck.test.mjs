...
test('content validator rejects duplicate events, unsafe source URLs, and unapproved image models',()=>{assert.doesNotThrow(()=>validateCard(card));assert.throws(()=>validateCollection([card,{...card,id:'different'}]),/重复事件/);assert.throws(()=>validateCard({...card,sources:[{name:'bad',url:'javascript:alert(1)'},card.sources[1]]}),/HTTPS/);assert.throws(()=>validateCard({...card,image:{...card.image,kind:'generated',model:'other',visualChecked:true}}),/GPT Image 2.5/);});
